...
def heightT(h):
    if 'cm' in h:
        if 150<=int(h.replace('cm', '')) <=193:
            return True
    elif 'in' in h:
        if 59 <= int(h.replace('in', '')) <= 76:
            return True
    return False
...
td = {
    ...
    'hgt' : heightT,
    'hcl' : lambda x: (x[0]=='#') and (len(x) == 7) and (all(map(lambda u: u in HCL, x[1:]))),
    'ecl' : lambda x: x in EYECOL,
    'pid' : lambda x: x.isdigit() and len(x) == 9,
    'cid' : lambda:1
}


def testField(key, field):
    '''
